# 人工待办清单（第十七轮，2026-08-11）

> 给老板/有账号同事的手动执行清单，按 ROI 排序。承接 `docs/ops/manual-todo-round16.md`，未完成项全部保留。通用文案见 `docs/promo/round3/`（如无则用 round10 目录站文案，模板数口径 **222 款**）。

## P0 — 四大站长平台注册验证（约 30 分钟，历史 P0，连续八轮未破口）

第十七轮复查（证据 `docs/ops-round17-evidence/`）：本轮出口 IP 100.23.34.160，**百度总查询恢复可核验——总收录仍约 1 条且仅首页**（round14 基线未变），/vs 精查仍被滑块验证码拦截；搜狗点选验证码、Google reCAPTCHA 拦截如旧；360 实测仍为 **0**；Bing "About 33,000 results" 首屏全是英文词典等无关结果（兜底污染，不可作证据）。/vs 矩阵与 /desk-card-generator、/name-card-batch 两个长尾落地页在可核验引擎（百度、360）中**均未收录**。331 个页面放量只差站长平台主动提交：
...
21. **优先发布第十七批素材**：`docs/promo/round17/`（小红书 1 篇、知乎 1 篇、公众号 1 篇，主打四大新卖点：**AI 自动设计**（文案已按 #216 降级口径写好——限量、繁忙时可能失败、推荐自定义 API，请勿自行加码承诺）、**反馈直达企微群**、**键盘-only 全流程可用**、**坏页零交付**）；第十六批（可靠性叙事+组合字段）、第十五批（少数民族文字/生僻字）及此前各轮未发布素材同样有效。

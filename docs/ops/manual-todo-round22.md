# 人工待办清单（第二十二轮，2026-08-11）

> 给老板/有账号同事的手动执行清单，按 ROI 排序。承接 `docs/ops/manual-todo-round21.md`，未完成项全部保留。通用文案见 `docs/promo/round3/`（如无则用 round10 目录站文案，模板数口径 **222 款**）。

## P0 — 五大站长平台注册验证（约 30 分钟，历史 P0，连续十三轮未破口）

第二十二轮复查（证据 `docs/ops-round22-evidence/`）：本轮出口 IP 54.201.200.193，**百度总收录仍约 1 条且仅首页**（round14 基线未变），搜狗点选验证码拦截如旧；360 实测仍为 **0**；神马本轮被阿里云盾（cloud_ip_bl）拦截无法核验（沿用 round21 实测 0 基线）；Bing "About 50 results" 首屏全是 courses.lumenlearning.com 等无关结果（兜底污染，不可作证据）。/vs 矩阵与 /desk-card-generator、/name-card-batch 两个长尾落地页在可核验引擎（百度、360）中**均未收录**。331 个页面放量只差站长平台主动提交：

1. **百度站长平台**（ziyuan.baidu.com）：注册/登录 → 添加 `www.seatmark.cn` → 验证 → 提交 sitemap（331 条）→ 开通普通收录 API 并保存 token。**百度连续多轮确认仅收录首页，验证后放量收益最直接。**
2. **Bing Webmaster Tools**（bing.com/webmasters）：Microsoft 账号 → 添加站点 → 提交 sitemap。IndexNow 已连续多轮全端点成功（本轮 331 条五端点全绿，2026-08-11 14:09 UTC）。
...
5. **神马站长平台（zhanzhang.sm.cn）**：注册 → 验证 → 提交 sitemap。神马 round21 实测整站收录 0（本轮数据中心 IP 被云盾拦截无法复查），为 UC/夸克移动流量入口，验证后同样有从 0 到 1 收益。
6. **本地网络 site: 复查并截图**：用家庭/手机网络查 `site:www.seatmark.cn`（百度/Google/搜狗/必应/神马）并截图发回，作为下一轮基线；重点确认 `/vs`、`/desk-card-generator`、`/name-card-batch` 是否已被收录。**神马侧尤其需要本地网络**（VM 数据中心 IP 已进云盾黑名单）。
...
22. **优先发布第二十二批素材**：`marketing/batch22/`（知乎 1 篇、小红书 1 篇、公众号 1 篇，主打**断网可用四卖点**：完全断网 300 张导出成功、弱网加载提示不白屏、200% 放大不破版导出照常、深色模式护设计原色+尊重减少动态）；第二十一批 `marketing/batch21/`（坏情况容错）与第二十批 `marketing/batch20/`（三引擎兼容）仍建议发布；此前各轮未发布素材同样有效。

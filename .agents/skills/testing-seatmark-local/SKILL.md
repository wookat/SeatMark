...
- PNG export (图片 PNG button): eink800 template defaults to 精确像素 800×480 + 纯黑白; verify outputs with Pillow (exact size, gray values ⊆ {0,255}, distinct md5 per page). Note: 纯黑白 binarization (threshold 160) erases the semi-transparent watermark, so watermarked-mono and clean-mono PNGs are pixel-identical — compare watermark presence only with 纯黑白 unchecked. Reset anonymous quota between runs via `localStorage.removeItem('seatmark.clean-export-usage.v1')` + reload.

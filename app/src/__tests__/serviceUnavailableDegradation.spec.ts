...
describe('第 345 轮：账号服务不可用降级文案', () => {
  ...
  describe('PreviewArea 导出选择框', () => {
    ...
    it('serviceUnavailable=true：未登录提示改为中性文案', async () => {
      ...
      // 第 370 轮：提示句独立于两张卡之外，误点不会触发无水印导出（不耗配额）
      const cleanBtn = wrapper.find('[data-testid="choose-clean"]')
      expect(cleanBtn.element.contains(hint.element)).toBe(false)
      expect(wrapper.find('[data-testid="choose-watermark"]').element.contains(hint.element)).toBe(false)
      expect(hint.element.closest('button')).toBeNull()
      expect(hint.element.tagName).toBe('P')
      expect(cleanBtn.element.compareDocumentPosition(hint.element) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()

...
async function onDownload() {
  try {
    ...
    toast.success('校准页已下载', '请用目标打印机按 100% 实际大小、无边距打印这一页')
  } catch (err) {
    toast.danger('校准页生成失败', err instanceof Error ? err.message : String(err))
  }
}
...
function onSave() {
  ...
  toast.success(
    '打印校准已保存',
    '之后所有导出与打印会自动应用偏移与缩放补偿，仅存于本机浏览器',
  )
  emit('close')
}
...
function onReset() {
  ...
  toast.info('已清除校准', '导出与打印恢复为不做任何补偿')
}
...
<template>
  <ModalDialog :open="open" title="打印校准向导" size="lg" @close="emit('close')">
    <div class="space-y-4">
      <p class="leading-6">
        打印跑偏、尺寸不准多因打印机存在固有偏移或缩放误差。按下面三步做一次校准，
        补偿参数保存在本机浏览器，之后所有导出与打印自动应用。
        详细排障说明见<a href="/guides/print-offset-calibration-wizard" target="_blank" class="font-semibold text-brand-600 hover:underline">打印偏移排障教程</a>。
      </p>
      ...
      <div class="rounded-lg border border-slate-200 bg-slate-50 p-4">
        ...
        <div class="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <label class="block">
            <span class="mb-1 block text-xs font-semibold text-slate-600">左边距</span>
            <NumberField v-model="measured.left" :step="0.5" :min="0" />
          </label>
          <label class="block">
            <span class="mb-1 block text-xs font-semibold text-slate-600">上边距</span>
            <NumberField v-model="measured.top" :step="0.5" :min="0" />
          </label>
          <label class="block">
            <span class="mb-1 block text-xs font-semibold text-slate-600">框宽</span>
            <NumberField v-model="measured.frameWidth" :step="0.5" :min="1" />
          </label>
          <label class="block">
            <span class="mb-1 block text-xs font-semibold text-slate-600">框高</span>
      ...
      <div class="rounded-lg border border-slate-200 bg-slate-50 p-4">
        ...
        <div class="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-slate-600 sm:grid-cols-4">
          <span>水平偏移 <strong class="text-slate-900">{{ fmtMm(result.offsetX) }}</strong></span>
          <span>垂直偏移 <strong class="text-slate-900">{{ fmtMm(result.offsetY) }}</strong></span>
        ...
        <p v-else-if="!resultReasonable" class="mt-2 text-xs leading-5 text-amber-600">
          计算出的补偿超出常见打印机误差范围，请复核量取值与打印缩放设置（必须为 100%）后重试。
        </p>
      </div>
      ...
      <div v-if="calibrationStore.active" class="flex items-center gap-2 rounded-lg bg-emerald-50 px-3 py-2 text-xs text-emerald-700">
        ...
        当前已有生效的校准（偏移 {{ fmtMm(calibrationStore.calibration.offsetX) }} / {{ fmtMm(calibrationStore.calibration.offsetY) }}，缩放 {{ fmtScale(calibrationStore.calibration.scaleX) }} / {{ fmtScale(calibrationStore.calibration.scaleY) }}），保存将覆盖。
      </div>
    </div>

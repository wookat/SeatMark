...
import SelectField, {
  type SelectOption,
} from "@/components/ui/SelectField.vue";
...
const pageWidthPx = computed(
  () => workspace.template.page.paperWidth * MM_TO_PX,
);
const pageHeightPx = computed(
  () => workspace.template.page.paperHeight * MM_TO_PX,
);
...
const hostWatermark = computed(
  () =>
    withWatermark.value &&
    !(pendingAction.value === "png" && pngMonochrome.value),
);
...
const pngTotalLabels = computed(() =>
  workspace.pages.reduce(
    (sum, page) => sum + page.filter((row) => row != null).length,
    0,
  ),
);
...
watch(pngNameMode, (mode) => {
  if (
    ...
    workspace.excel.headers.length
  ) {
...
const PNG_SIZE_OPTIONS = computed<SelectOption[]>(() => [
  ...
  {
    value: "exact",
    label: isEinkTemplate.value
      ? "精确像素（电子墨水屏 800×480）"
      : "精确像素（自定义宽度）",
  },
]);

const pngExactWidthValid = computed(() =>
  isValidExactPixelWidth(pngExactWidth.value),
);
...
const pngExactHeight = computed(() => {
  ...
  return exactPixelHeight(
    pngExactWidth.value,
    pngDesignRect.value.width,
    pngDesignRect.value.height,
  );
});
...
function saveEditOne() {
  ...
  for (const field of workspace.mappableFields) {
    ...
    if (value !== workspace.baseFieldText(row, field.id))
      override[field.id] = value;
  }
...
async function doExportPdf() {
  ...
  try {
    ...
    const scale = adaptiveRasterScale(
      pageCount,
      workspace.renderTemplate.label,
    );
    await exportPagedPdf({
      ...
      pageWidth: workspace.template.page.paperWidth,
      pageHeight: workspace.template.page.paperHeight,
      calibration: calibrationStore.active
        ? calibrationStore.calibration
        : undefined,
...
async function doExportPng() {
  if (
    pngSizeMode.value === "exact" &&
    !pngPreset.value &&
    !pngExactWidthValid.value
  ) {
    toast.warning(
      "像素宽度无效",
      `请输入 ${MIN_EXACT_PIXEL_WIDTH}–${MAX_EXACT_PIXEL_WIDTH} 之间的整数像素宽度`,
    );
    return;
  }
  const abort = new AbortController();
  ...
  try {
    ...
    if (perLabel) {
      ...
      if (pngNameMode.value === "field" && pngNameTemplate.value.trim()) {
        ...
        let n = 0;
        for (const items of labelsByPage)
          for (const item of items) item.fileName = names[n++];
      }
    }
    // 按字段命名（整页成图）：多枚模板每页取第一条记录求值；空模板/空字段回退序号命名
    const pageFileNames =
      !perLabel &&
      pngNameMode.value === "field" &&
      pngNameTemplate.value.trim() &&
      pageCount > 1
        ? buildFieldFileNames({
            template: pngNameTemplate.value.trim(),
            rows: workspace.pages.map(
              (page) => page.find((row) => row != null) ?? null,
            ),
            fallbackPrefix: baseName,
          })
        : undefined;
    ...
    await exportPagedPng({
      ...
      exactPixels: exact
        ...
        : undefined,
      cropRect:
        exact && !perLabel ? (pngCropRect.value ?? undefined) : undefined,
      labelsByPage,
      monochrome: pngMonochrome.value,
      watermarkText:
        withWatermark.value && pngMonochrome.value
    ...
    toast.success(
      unitCount === 1
        ? "PNG 图片已生成"
        : `PNG 图片已生成（${unitCount} ${unitWord}打包为 zip）`,
      exact
...
const unmappedFieldIds = computed(
  () => new Set(workspace.unmappedFields.map((f) => f.id)),
);
...
<template>
  <section class="flex h-full flex-col">
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                ...
                <path
                  d="M6.4 6.2a1.6 1.6 0 1 1 2.4 1.4c-.5.3-.8.6-.8 1.2m0 2h.01"
                />
              </svg>
            </button>
          </span>
          ...
          <button
            ...
            :title="
              calibrationStore.active
                ? '打印校准已生效：导出与打印自动应用偏移/缩放补偿'
                : '打印跑偏、尺寸不准？打印一页标尺校准页，量两下即可全局补偿'
            "
          ...
          >
            ...
            >
              <path
                d="M3 17 17 3l4 4L7 21H3v-4zM14 6l4 4M9 11l1.5 1.5M11.5 8.5 13 10"
              />
            </svg>
            打印校准<span
        ...
        >
          ...
          >
            <path
              d="M7 8V3h10v5M7 17H4a1 1 0 0 1-1-1V9a1 1 0 0 1 1-1h16a1 1 0 0 1 1 1v7a1 1 0 0 1-1 1h-3m-10-3h10v7H7v-7z"
            />
          </svg>
    ...
    >
      <span
        class="flex size-7 shrink-0 items-center justify-center rounded-lg bg-emerald-600 text-white"
      >
        <svg
          class="size-3.5"
          viewBox="0 0 24 24"
      ...
      <p class="min-w-0 flex-1 text-xs leading-5 text-slate-600">
        <strong class="text-slate-800">{{
          sharePromptWatermarked ? "想去除页脚水印？" : "成品已拿到，觉得好用？"
        }}</strong>
        {{
      ...
      <div
        class="flex shrink-0 items-center gap-1.5 max-sm:basis-full max-sm:justify-end"
      >
        <button
    ...
    >
      ...
      <div v-else class="flex justify-center">
        ...
        >
          <span
            class="mt-0.5 flex size-5 shrink-0 items-center justify-center rounded-full bg-brand-100 text-brand-600"
          >
            <svg
              class="size-3"
              viewBox="0 0 24 24"
    ...
    >
      <div
        v-if="pendingAction === 'png'"
        class="mb-3 rounded-lg border border-slate-200 bg-slate-50/70 p-3"
      >
        <label class="field-label" for="png-unit">成图单位</label>
        <SelectField
          id="png-unit"
          v-model="pngExportUnit"
          size="sm"
          :options="PNG_UNIT_OPTIONS"
        />
        <p class="mt-1.5 text-xs leading-5 text-slate-600">
          {{
            pngExportUnit === "label"
        ...
        <SelectField
          v-model="pngSizeMode"
          size="sm"
          :options="PNG_SIZE_OPTIONS"
        />
        <template v-if="pngSizeMode === 'exact'">
          <label class="field-label mt-2" for="png-preset"
            >分辨率预设（电子墨水屏）</label
          >
          <SelectField
            id="png-preset"
            v-model="pngPresetId"
            size="sm"
            :options="PNG_PRESET_OPTIONS"
          />
          <div
            v-if="!pngPreset"
            class="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-600"
          >
            <label class="font-semibold" for="png-exact-width"
              >宽度（px）</label
            >
            <input
              id="png-exact-width"
            ...
            <span v-if="pngExactWidthValid" class="text-slate-600">
              ...
              <strong class="text-slate-700"
                >{{ pngExactWidth }}×{{ pngExactHeight }}</strong
              >
              像素（高度按模板比例自动推导）
            </span>
            <span v-else class="text-red-500"
              >请输入 {{ MIN_EXACT_PIXEL_WIDTH }}–{{
                MAX_EXACT_PIXEL_WIDTH
              }}
              之间的整数</span
            >
          ...
          <p v-else class="mt-2 text-xs text-slate-600">
            每页精确输出
            <strong class="text-slate-700"
              >{{ pngPreset.width }}×{{ pngPreset.height }}</strong
            >
            像素
          </p>
          <p
            v-if="pngPresetStretch"
            class="mt-1.5 rounded-lg bg-amber-50 px-3 py-2 text-xs leading-5 text-amber-700"
          >
        ...
        <p class="mt-2 text-xs leading-5 text-slate-600">
          单张直接下载 PNG，多张自动打包为 zip；{{
            isEinkTemplate
              ? "电子座签模板默认精确 800×480 像素 + 纯黑白，可直接导入电子桌牌系统。"
              : "需要精确像素（如电子墨水屏）时选「精确像素」，可用分辨率预设或自定义宽度。"
          }}
        </p>
        <template
          v-if="
          ...
          "
        >
          <label class="field-label mt-3" for="png-name-mode"
            >zip 内文件命名</label
          >
          <SelectField
            id="png-name-mode"
            v-model="pngNameMode"
            size="sm"
            :options="PNG_NAME_OPTIONS"
          />
          <div
            v-if="pngNameMode === 'field'"
          ...
          >
            ...
            />
            <div
              v-if="workspace.excel.headers.length"
            ...
            <p v-else class="mt-1.5 text-slate-600">
              {列名} 会替换为{{
                pngExportUnit === "label"
                  ? "每张标签对应名单行"
                  : "该页对应名单行（每页多枚时取该页第一条记录）"
              }}的内容；非法字符自动过滤，重名自动追加 -2，空字段回退序号命名。
            </p>
          </div>
        </template>
      </div>
      ...
      >
        图片版
        PDF：每页渲染为高清图片后合成，所见即所得、任何设备打开都一致；文字不可选中，需要矢量文字请改用「打印
        / 矢量 PDF」。
      </p>
      <p
      ...
      >
        <template v-if="isMobile">
          手机浏览器的打印预览容易出现空白：将先生成打印用
          PDF，再调起系统分享面板，在面板中选「打印」或用 PDF 应用打开后打印。
        </template>
        <template v-else>
          打印 / 矢量 PDF：调起浏览器打印对话框，目标打印机选「另存为
          PDF」即得到文字可选中的矢量 PDF，也可直接连打印机输出。
        </template>
      </p>
      <details
      ...
      >
        <summary
          class="cursor-pointer text-xs font-bold text-amber-800 select-none"
        >
          打印出来是黑白的？彩色打印检查清单（3 步）
        </summary>
        <ol
          class="mt-2 list-decimal space-y-1.5 pl-5 text-xs leading-5 text-slate-600"
        >
          <li>
            在浏览器打印对话框展开「更多设置」，勾选
            <strong class="text-slate-800"
              >「背景图形 / Background graphics」</strong
            >，否则背景色与装饰会整体丢失。
          </li>
          <li>
            打印对话框的「颜色」选项选
            <strong class="text-slate-800">「彩色 / Color」</strong
            >，不要选黑白（黑白模式会把整页转成灰度）。
          </li>
          <li>
            仍是黑白时，多半是打印机驱动默认开了省墨 /
            灰度模式：在打印对话框的「使用系统对话框打印」或系统「打印机首选项」里，关闭
            <strong class="text-slate-800"
              >「灰度打印 / 黑白打印 / 省墨模式」</strong
            >后重试。
          </li>
        </ol>
        <p class="mt-1.5 pl-5 text-[11px] leading-4 text-slate-600">
          小验证：目标打印机先选「另存为 PDF」，导出的 PDF
          是彩色就说明页面没问题，剩下的是打印机设置。
        </p>
      </details>
      ...
      >
        共 {{ exportEstimate.pageCount }} 页 · 每页约
        {{ exportEstimate.dpi }}dpi · 预估体积约
        <span class="font-bold text-slate-700">{{ exportEstimate.size }}</span
        >（按页数自适应清晰度与压缩）
      </p>
      <div class="mt-3 grid gap-3">
        ...
        >
          ...
          <span>
            <span class="block text-sm font-bold text-slate-900"
              >无水印导出（今日剩余 {{ quota.remaining }} 次）</span
            >
        ...
        >
          <span
            class="flex size-8 shrink-0 items-center justify-center rounded-lg bg-slate-600 text-white"
          >
            <svg
              class="size-4"
              viewBox="0 0 24 24"
          ...
          <span>
            ...
            >
            <span class="mt-0.5 block text-xs leading-5 text-slate-600">
              每张标签内底部叠加半透明徽章式品牌水印（座位格标记 +
              seatmark.cn，配色随模板自适应），不遮挡姓名等核心内容
            </span>
          </span>
        </button>
      </div>
      <p v-if="!auth.isLoggedIn" class="mt-3 text-xs leading-5 text-slate-600">
        登录后无水印导出每天 {{ QUOTA_USER_DAILY }} 次，分享链接每被点开 1
        次再得 1 次；同时获得专业版 Beta 限时免费试用。
      </p>
    </ModalDialog>
    ...
    <Teleport to="body">
      <div v-if="renderHost" ref="hostRef" class="offscreen-host">
        <LabelSheet
          ...
          :get-photo="workspace.photoFor"
          :show-cut-lines="
            hostSuppressCutLines ? false : workspace.showCutLines
          "

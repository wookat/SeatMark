...
import { localePath, t as tr, useI18n } from '@/i18n'
...
const searchScopeNote = computed(() => {
  ...
  if (searchResult.value.globalFallback) {
    return `${tr('「')}${tr(activeCategoryName.value)}${tr('」分类下无匹配，已在全部分类中找到')} ${filteredItems.value.length} ${tr('款')}`
  }
  if (activeCategory.value !== 'all') {
    return `${tr('在「')}${tr(activeCategoryName.value)}${tr('」分类中找到')} ${filteredItems.value.length} ${tr('款')}`
  }
  return `${tr('在全部分类中找到')} ${filteredItems.value.length} ${tr('款')}`
})
  ...
  <div class="mx-auto w-full max-w-6xl px-4 py-10 sm:py-14">
    <div class="text-center">
      ...
      <p class="mx-auto mt-3 max-w-2xl text-sm leading-6 text-slate-600">
        {{ items.length }}
        {{ tr('款免费内置模板覆盖考场座签、考号贴、课桌姓名贴、会议桌牌、出入证、学生证、工作证等场景，全部以毫米为单位精确排版，点击任意模板查看详情，或直接开始生成。') }}
      </p>
      <p
        v-if="locale === 'en'"
        class="mx-auto mt-3 max-w-2xl rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs leading-5 text-slate-600"
      >
        Template names and descriptions are currently in Chinese; the Studio UI is available in English.
      </p>
    </div>
    ...
    <div class="mt-8 flex flex-col items-center gap-3">
      <label class="relative block w-full max-w-md">
        ...
        <input
          v-model="searchQuery"
          type="search"
          :placeholder="tr('搜索模板 / 场景，支持拼音、首字母')"
    ...
    >
      ...
      <div class="mt-6 border-t border-slate-200 pt-5 text-left">
        <p class="text-xs font-bold text-slate-600">{{ tr('也许这些模板能满足需求') }}</p>
        <div class="mt-3 grid gap-4 sm:grid-cols-3">
          <RouterLink
            v-for="rec in recommendedItems"
            :key="rec.detail.slug"
            :to="localePath(`/templates/${rec.detail.slug}`)"
    ...
    <div class="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
      <RouterLink
        ...
        :to="localePath(`/templates/${item.detail.slug}`)"
      ...
      >
        ...
        <div class="flex flex-1 flex-col p-4">
          ...
          <div class="mt-auto flex flex-wrap gap-1.5 pt-3">
            ...
            <span class="rounded-md bg-slate-100 px-1.5 py-0.5 text-[10px] font-semibold text-slate-600">
              {{ item.template!.page.cols * item.template!.page.rows }} {{ tr('枚 / 页') }}
            </span>
          </div>
        </div>
      </RouterLink>
    ...
    >
      <div>
        <h2 class="text-base font-bold text-slate-900">{{ tr('没有完全合适的？') }}</h2>
        <p class="mt-1 text-sm text-slate-600">
          {{ tr('任何模板都可以在可视化设计器中继续调整，也可以从空白开始完全自定义。') }}
        </p>
      </div>
      <RouterLink
        :to="localePath('/studio?design=new')"

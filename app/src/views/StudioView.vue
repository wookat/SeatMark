...
<template>
  <div class="mx-auto w-full max-w-[1480px] px-4 py-5">
    <!-- 紧凑可见标题：最多两行不省略、不吸顶，移动端位于吸顶分段条上方，≥md 与设置列同宽 -->
    <h1
      class="mb-1 line-clamp-2 text-sm leading-5 font-semibold text-balance text-slate-800 sm:leading-6 md:w-[320px] lg:w-[400px]"
      data-testid="studio-title"
    >
      {{ locale === 'en' ? 'SeatMark Studio: place cards, seat labels & name tags in bulk' : 'SeatMark 标签工坊：批量生成桌牌 / 座签 / 席位卡' }}
    </h1>

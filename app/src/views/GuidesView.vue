  ...
  <div class="mx-auto w-full max-w-6xl px-4 py-10 sm:py-14">
    <div class="text-center">
      ...
      <p class="mx-auto mt-3 max-w-2xl text-sm leading-6 text-slate-600">
        {{ tr('座签、桌牌、席位卡、证卡的制作与打印实战教程：从 Excel 名单整理、模板选择到打印裁切，问答式讲解常见坑点，看完即可上手。') }}
      </p>
      <p
        v-if="locale === 'en'"
        class="mx-auto mt-3 max-w-2xl rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs leading-5 text-slate-600"
      >
        Guides are currently in Chinese; the Studio UI is available in English.
      </p>
    </div>
    ...
    <div class="mt-8 grid gap-3">
      <label class="relative mx-auto block w-full max-w-md">
        ...
        <input
          v-model="searchQuery"
          type="search"
          :placeholder="tr('搜索教程，支持拼音、首字母，如“打印”“dayin”“jkz”')"
      ...
      <div class="flex flex-wrap items-center gap-2">
        <span class="shrink-0 text-xs font-bold text-slate-600">{{ tr('主题') }}</span>
        <button
      ...
      <div class="flex flex-wrap items-center gap-2">
        <span class="shrink-0 text-xs font-bold text-slate-600">{{ tr('群体') }}</span>
        <button
      ...
      <p class="text-xs text-slate-600">{{ tr('共') }} {{ filteredGuides.length }} {{ tr('篇教程') }}</p>
    </div>
    ...
    >
      <p class="text-sm text-slate-600">{{ tr('该条件下暂无教程，换个关键词或筛选条件试试。') }}</p>
      <button
        type="button"
      ...
      <div class="mt-6 border-t border-slate-200 pt-5 text-left">
        <p class="text-xs font-bold text-slate-600">{{ tr('也许你想看这些教程') }}</p>
        <div class="mt-3 grid gap-3 sm:grid-cols-3">
          <RouterLink
            v-for="rec in recommendedGuides"
            :key="rec.slug"
            :to="localePath(`/guides/${rec.slug}`)"
    ...
    <div v-else class="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
      ...
      >
        <div class="flex items-center gap-2 text-[11px] font-bold">
          ...
          <span class="text-slate-600">{{ tr('约') }} {{ guide.readingMinutes }} {{ tr('分钟读完') }}</span>
        </div>
        <h2
    ...
    >
      <div>
        <h2 class="text-base font-bold text-slate-900">{{ tr('边看边做，效果最好') }}</h2>
        <p class="mt-1 text-sm text-slate-600">
          {{ tr('打开标签工坊，用演示数据即可完整体验教程里的每一步，全程免费。') }}
        </p>
      </div>
      <RouterLink
